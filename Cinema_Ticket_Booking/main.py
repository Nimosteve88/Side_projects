...
class Card:
    ...
    def validate(self, price):  # checks if card is valid and has balance
        connection = sqlite3.connect(self.database)
        cursor = connection.cursor()
        cursor.execute("""
...
class Ticket:
    ...
    def to_pdf(self):
        ...
        pdf.cell(w=100, h=25, txt="Your Digital Ticket", border=1)

        pdf.set_font(family="Times", style="B", size=14)
        pdf.cell(w=100, h=25, txt="Name: ", border=1)
        pdf.set_font(family="Times", style="B", size=12)
        pdf.cell(w=100, h=25, txt=self.user.name, border=1, ln=1)
        pdf.cell(w=100, h=5, txt="", border=0, ln=1)

        pdf.set_font(family="Times", style="B", size=14)
        pdf.cell(w=100, h=25, txt="Ticket ID", border=1)
        pdf.set_font(family="Times", style="B", size=12)
        pdf.cell(w=100, h=25, txt=self.id, border=1, ln=1)
        pdf.cell(w=100, h=5, txt="", border=0, ln=1)

        pdf.set_font(family="Times", style="B", size=14)
        pdf.cell(w=100, h=25, txt="Price", border=1)
        pdf.set_font(family="Times", style="B", size=12)
        pdf.cell(w=100, h=25, txt=str(self.price), border=1, ln=1)
        pdf.cell(w=100, h=5, txt="", border=0, ln=1)

        pdf.set_font(family="Times", style="B", size=14)
        pdf.cell(w=100, h=25, txt="Seat Number", border=1)
        pdf.set_font(family="Times", style="B", size=12)
        pdf.cell(w=100, h=25, txt=str(self.seat_number), border=1, ln=1)
        pdf.cell(w=100, h=5, txt="", border=0, ln=1)
